...
export async function Contributors() {
  ...
  try {
    const res = await fetch('https://api.github.com/repos/RamonSantos9/ProjetoAds/contributors', {
  ...
  return (
    <div className="flex flex-col items-center border-x border-t border-dashed px-4 py-16 text-center bg-fd-background/50">
      ...
      <p className="mb-4 text-fd-muted-foreground">
        O <span className="font-bold text-fd-foreground">PodcastAds</span> evolui a cada dia graças ao talento e dedicação da nossa comunidade.
      </p>
        ...
        {contributors.length > 0 ? (
          <div className="flex flex-row flex-wrap items-center justify-center md:pe-4">
            ...
            {contributors.length > 20 && (
              <div 
                className="size-10 md:size-12 flex items-center justify-center rounded-full bg-fd-secondary text-fd-secondary-foreground border-4 border-fd-background text-xs font-bold md:p-4 z-0"
              >
                +{contributors.length - 20}
              </div>
            )}
          ...
          <div className="text-sm text-fd-muted-foreground italic font-mono">
            Os nomes das pessoas que contribuem para o projeto serão exibidos em breve.
          </div>
        )}
