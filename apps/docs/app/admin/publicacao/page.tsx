          ...
          <section className="flex flex-col gap-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <p className="text-[#74748D] dark:text-fd-muted-foreground text-sm max-w-2xl">
                Monitore e execute a distribuição dos seus episódios em múltiplas redes. Garanta que o SEO e as metatags estejam otimizados antes de publicar.
              </p>
              <ActionButtonRefined 
                label="Publicar Novo" 
                icon={<Zap className="size-5" />}
                onClick={() => alert('Selecione um episódio para iniciar o workflow de publicação.')}
              />
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
               {[
                 { name: 'Spotify', icon: <Music className="size-5 text-[#1DB954]" />, count: '12 ativações' },
                 { name: 'YouTube', icon: <Youtube className="size-5 text-[#FF0000]" />, count: '8 vídeos' },
                 { name: 'Instagram', icon: <Instagram className="size-5 text-[#E4405F]" />, count: '24 cortes' },
                 { name: 'RSS Feed', icon: <Radio className="size-5 text-[#FFA500]" />, count: 'PodcastAds XML' },
               ].map((p, i) => (
                 <div key={i} className="p-4 rounded-xl border border-fd-border bg-fd-accent/30 hover:bg-fd-accent/50 transition-colors cursor-pointer group">
                   <div className="flex items-center gap-3 mb-2">
                     {p.icon}
                     <span className="font-bold text-sm text-fd-foreground">{p.name}</span>
                   </div>
                   <p className="text-[10px] text-fd-muted-foreground">{p.count}</p>
                 </div>
               ))}
            </div>
            ...
            <div className="grid gap-4 mt-2">
               {loading ? (
                 Array.from({ length: 3 }).map((_, i) => (
                   <div key={i} className="h-24 rounded-xl border border-fd-border bg-fd-accent animate-pulse" />
                 ))
               ) : filtered.length === 0 ? (
                 ...
                 filtered.map(ep => (
                   <div key={ep.id} className="flex flex-col md:flex-row md:items-center justify-between p-5 rounded-xl border border-fd-border bg-white dark:bg-[#121212] hover:border-fd-primary/50 transition-all gap-4">
                     <div className="flex gap-4 items-start min-w-0">
                        ...
                        <div className="min-w-0">
                           <h3 className="font-bold text-fd-foreground truncate">{ep.title}</h3>
                           <p className="text-xs text-fd-muted-foreground line-clamp-1 mt-0.5">{ep.summary}</p>
                           <div className="flex items-center gap-3 mt-2">
                              <span className="text-[10px] font-medium text-fd-muted-foreground uppercase flex items-center gap-1">
                                <Clock className="size-3" /> {new Date(ep.createdAt).toLocaleDateString()}
                              </span>
                              <span className={cn(
                                "text-[10px] font-bold px-2 py-0.5 rounded-full",
                                ep.status === 'Publicado' ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400"
                              )}>
                                {ep.status}
                              </span>
                           </div>
                     ...
                     <div className="flex items-center gap-6">
                        ...
                        <div className="flex items-center gap-4">
                          <TooltipRefined note="Spotify">
                            <div className={cn("p-1.5 rounded-full border", ep.platforms.includes('Spotify') ? "bg-green-100 border-green-200 text-green-600 dark:bg-green-900/20 dark:border-green-800" : "bg-fd-accent border-fd-border text-fd-muted-foreground opacity-30")}>
                               <Music className="size-4" />
                            </div>
                          </TooltipRefined>
                          <TooltipRefined note="YouTube">
                            <div className={cn("p-1.5 rounded-full border", ep.platforms.includes('YouTube') ? "bg-red-100 border-red-200 text-red-600 dark:bg-red-900/20 dark:border-red-800" : "bg-fd-accent border-fd-border text-fd-muted-foreground opacity-30")}>
                               <Youtube className="size-4" />
                            </div>
                          </TooltipRefined>
                          <TooltipRefined note="Instagram">
                            <div className={cn("p-1.5 rounded-full border", ep.platforms.includes('Instagram') ? "bg-pink-100 border-pink-200 text-pink-600 dark:bg-pink-900/20 dark:border-pink-800" : "bg-fd-accent border-fd-border text-fd-muted-foreground opacity-30")}>
                        ...
                        <button className="flex items-center gap-2 px-4 py-2 bg-fd-primary text-white rounded-lg text-sm font-bold hover:opacity-90 transition-opacity">
                           {ep.status === 'Publicado' ? 'Re-publicar' : 'Publicar Agora'}
                        </button>
...
function TooltipRefined({ children, note }: { children: React.ReactNode, note: string }) {
  return (
