...
export default function RoteirosAdminPage() {
  ...
  const getStatusColor = (status: ScriptStatus) => {
    switch (status) {
      ...
      default:
        return 'text-fd-muted-foreground bg-fd-accent border-fd-border';
    }
  };
          ...
          <section className="flex flex-col gap-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <p className="text-[#74748D] dark:text-fd-muted-foreground text-sm max-w-2xl">
                Crie, edite e revise os roteiros dos seus episódios. Use o{' '}
                <strong>AI Assistant</strong> para gerar sugestões de perguntas
                e tópicos baseados no perfil do convidado.
              </p>
            ...
            <div className="flex items-center gap-8 py-2 overflow-x-auto no-scrollbar">
              <div className="flex flex-col">
                ...
                <span className="text-[10px] text-fd-muted-foreground uppercase tracking-wider font-bold">
                  Total Doc.
                </span>
              </div>
              ...
              <div className="flex flex-col">
                <span className="text-2xl font-bold text-orange-500">
                  {scripts.filter((s) => s.status === 'Em Redação').length || 0}
                </span>
                <span className="text-[10px] text-fd-muted-foreground uppercase tracking-wider font-bold">
                  Em Redação
                </span>
              ...
              <div className="flex flex-col">
                <span className="text-2xl font-bold text-blue-500">
                  {scripts.filter((s) => s.status === 'Revisão').length || 0}
                </span>
                <span className="text-[10px] text-fd-muted-foreground uppercase tracking-wider font-bold">
                  Revisão
                </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-2">
              {loading ? (
                Array.from({ length: 6 }).map((_, i) => (
                  <div
                    key={i}
                    className="h-48 rounded-xl border border-fd-border bg-fd-accent animate-pulse"
                  />
                ))
              ) : filtered.length === 0 ? (
                ...
                filtered.map((script) => (
                  <div
                    key={script.id}
                    className="group relative bg-white dark:bg-[#121212] border border-fd-border rounded-2xl p-5 hover:border-fd-primary/40 hover:shadow-lg transition-all flex flex-col overflow-hidden"
                  >
                    ...
                    <h3 className="font-bold text-fd-foreground mb-1 line-clamp-2 pr-8">
                      {script.title}
                    </h3>
                    <p className="text-[10px] text-fd-muted-foreground mb-6 italic">
                      Autor: {script.author} • v{script.version}
                    </p>

                    <div className="mt-auto pt-4 border-t border-fd-border flex items-center justify-between">
                      <div className="flex flex-col">
                        <span className="text-[9px] text-fd-muted-foreground uppercase font-bold tracking-tight">
                          Última edição
                        </span>
