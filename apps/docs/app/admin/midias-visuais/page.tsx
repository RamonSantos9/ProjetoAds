...
export default function MidiasAdminPage() {
  ...
  const getTypeIcon = (type: AssetType) => {
    switch (type) {
      ...
      case 'Banner':
        return <Layout className="size-4 text-blue-500" />;
      case 'Story':
        return <Type className="size-4 text-purple-500" />;
      default:
          ...
          <section className="flex flex-col gap-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <p className="text-[#74748D] dark:text-fd-muted-foreground text-sm max-w-2xl">
                Gerencie todos os elementos visuais da identidade do seu
                podcast. Capas automáticas, artes para redes sociais e
                apresentações.
              </p>
              <div className="flex items-center gap-2">
                <button className="flex items-center gap-2 px-4 py-2 bg-fd-accent text-fd-foreground rounded-lg text-sm font-bold border border-fd-border hover:bg-fd-accent/80 transition-all">
                  <Palette className="size-4 text-fd-primary" /> Gerador de
                  Capas
                </button>
                <ActionButtonRefined
                  label="Upload Asset"
              ...
              <div className="flex items-center gap-2 bg-fd-accent/30 p-1 rounded-lg border border-fd-border self-end md:self-auto">
                <button
                  ...
                  className={cn(
                    ...
                    view === 'grid'
                      ? 'bg-white dark:bg-[#1A1A1A] text-fd-primary shadow-sm'
            ...
            {view === 'grid' ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-2">
                {loading ? (
                  Array.from({ length: 8 }).map((_, i) => (
                    <div
                      key={i}
                      className="aspect-square rounded-2xl bg-fd-accent animate-pulse"
                    />
                  ))
                ) : filtered.length === 0 ? (
                  ...
                  filtered.map((asset) => (
                    <div
                      key={asset.id}
                      className="group relative flex flex-col bg-white dark:bg-[#121212] border border-fd-border rounded-2xl overflow-hidden hover:border-fd-primary/50 transition-all"
                    >
                      <div className="aspect-video relative overflow-hidden bg-fd-accent">
                        ...
                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                          <button
                            className="p-2 bg-white rounded-full text-black hover:bg-fd-primary hover:text-white transition-colors"
                            title="Visualizar"
                          >
                      ...
                      <div className="p-4 flex flex-col justify-between flex-1">
                        <div>
                          <h3 className="text-xs font-bold text-fd-foreground line-clamp-1 group-hover:text-fd-primary transition-colors">
                            {asset.title}
                          </h3>
                          <p className="text-[10px] text-fd-muted-foreground mt-1 uppercase tracking-tight font-medium">
              ...
              <div className="flex flex-col gap-2 mt-2">
                ...
                {filtered.map((asset) => (
                  <div
                    key={asset.id}
                    className="flex items-center justify-between p-3 border-b border-fd-border hover:bg-fd-accent/30 transition-colors"
                  >
                    ...
                    <div className="flex items-center gap-1">
                      <button className="p-2 hover:bg-fd-accent rounded text-fd-muted-foreground">
                        <Download className="size-4" />
                      </button>
                      <button className="p-2 hover:bg-fd-accent rounded text-fd-muted-foreground">
