...
export default function TranscricoesAdminPage() {
  ...
  const handleSimulateNew = () => {
     // No longer needed as we use real uploads via CreateEpisodeModal
     alert('O sistema agora é real! Use o botão de criar novo episódio no dashboard.');
  };
  ...
  const handleExport = (format: 'json' | 'csv' | 'txt') => {
    ...
    if (format === 'json') {
      ...
      const headers = ['ID', 'Episódio', 'Status', 'Confiança', 'Duração', 'Idioma', 'Data'];
          ...
          <section className="flex flex-col gap-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <p className="text-[#74748D] dark:text-fd-muted-foreground text-sm max-w-2xl">
                Gerencie e edite as transcrições geradas automaticamente via ElevenLabs AI.
              </p>
              <ActionButtonRefined 
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <div className="relative w-full sm:w-auto" ref={exportRef}>
                  ...
                  >
                    <Download className="size-4" /> Exportar Tudo <ChevronDown className={cn("size-3 transition-transform ml-1", isExportOpen ? "rotate-180" : "")} />
                  </button>
                  ...
                  {isExportOpen && (
                    <div className="absolute top-[calc(100%+4px)] right-0 w-full sm:w-48 bg-white dark:bg-[#121212] border border-[#E2E7F1] dark:border-fd-border rounded-lg shadow-xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-1">
                      ...
                      >
                        <FileJson className="size-4 text-fd-primary" /> JSON
                      </button>
                      <button 
                        onClick={() => handleExport('csv')}
                        className="w-full h-10 flex items-center gap-3 px-4 text-sm font-semibold text-[#0A1B39] dark:text-fd-foreground hover:bg-[#F6F8FA] dark:hover:bg-fd-accent border-b border-fd-border transition-colors text-left"
                      >
                        <Table className="size-4 text-fd-primary" /> CSV
                      </button>
                      <button 
                      ...
                      >
                        <FileText className="size-4 text-fd-primary" /> TXT
                      </button>
                    </div>
                  )}
                </div>
              ...
              <table className="w-full border-separate border-spacing-0 leading-[12px]">
                <thead>
                  <tr className="text-background text-sm">
                    <th className="bg-background border-y border-[#E2E7F1] dark:border-fd-border first:border-s first:rounded-s-xl last:rounded-e-xl last:border-e p-5 text-left whitespace-nowrap">Episódio</th>
                    <th className="bg-background border-y border-[#E2E7F1] dark:border-fd-border p-5 text-left whitespace-nowrap">Status</th>
                    <th className="bg-background border-y border-[#E2E7F1] dark:border-fd-border p-5 text-left whitespace-nowrap">Idioma</th>
                    <th className="bg-background border-y border-[#E2E7F1] dark:border-fd-border p-5 text-left whitespace-nowrap">Página Pública</th>
                ...
                <tbody>
                  ...
                  {loading ? (
                    Array.from({ length: 3 }).map((_, i) => (
                      <tr key={i}>
                        <td className="p-5 border-b border-fd-border"><Skeleton className="h-4 w-3/4" /></td>
                        <td className="p-5 border-b border-fd-border"><Skeleton className="h-6 w-20 rounded-full" /></td>
                        <td className="p-5 border-b border-fd-border"><Skeleton className="h-4 w-24" /></td>
                        <td className="p-5 border-b border-fd-border"><Skeleton className="h-4 w-12" /></td>
                        <td className="p-5 border-b border-fd-border text-right"><Skeleton className="h-10 w-10 ml-auto rounded-lg" /></td>
                      </tr>
                    ))
                  ) : filtered.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="text-center py-20 text-[#74748D]">Nenhuma transcrição encontrada.</td>
                    </tr>
                  ) : (
                    filtered.map(t => (
                      <tr key={t.id} className="hover:bg-background transition-colors group text-background">
                        <td className="p-5 border-b border-fd-border text-xs truncate">
                          <div className="flex flex-col gap-1">
                            <span className="font-bold text-background truncate max-w-[300px]">{t.title}</span>
                        ...
                        <td className="p-5 border-b border-fd-border text-xs">
                          <Badge 
                            variant={t.status === 'Concluído' ? 'success' : t.status === 'Processando' ? 'warning' : 'error'}
                          >
                            {t.status}
                          </Badge>
                        ...
                        <td className="p-5 border-b border-fd-border text-right text-xs">
                          <div className="flex items-center justify-end gap-2">
                            <button 
                              className="p-1.5 rounded-lg bg-background cursor-pointer"
                              onClick={(e) => {
                                const rect = e.currentTarget.getBoundingClientRect();
                                setMenuOpen(menuOpen?.id === t.id ? null : { id: t.id, rect });
                              }}
                            >
                              <ActionsIcon />
                            </button>
                            {menuOpen?.id === t.id && createPortal(
                                <div
