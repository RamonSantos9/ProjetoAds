...
export default function VinhetasAdminPage() {
  ...
  const sounds: SoundAsset[] = [
    {
      id: 's1',
      title: 'Intro PodcastAds Principal',
      type: 'Vinheta',
      duration: '0:15',
      url: '#',
      tags: ['Moderno', 'Abertura'],
    },
    {
      id: 's2',
      title: 'Background Lo-Fi Soft',
      type: 'Trilha',
      duration: '2:45',
      url: '#',
      tags: ['Entrevista', 'Fundo'],
    },
    {
      id: 's3',
      title: 'Efeito Transição Digital',
      type: 'Efeito',
      duration: '0:03',
      url: '#',
      tags: ['Corte', 'Rápido'],
    ...
    {
      id: 's4',
      title: 'Outro Encerramento Padrão',
      type: 'Vinheta',
      duration: '0:20',
      url: '#',
      tags: ['Finalização'],
          ...
          <section className="flex flex-col gap-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <p className="text-[#74748D] dark:text-fd-muted-foreground text-sm max-w-2xl">
                Biblioteca de áudio dedicada à sonorização dos seus episódios.
                Organize trilhas brancas, efeitos sonoros (SFX) e identidades
                vocais.
              </p>
              <ActionButtonRefined
                label="Novo Áudio"
            ...
            <div className="flex items-center gap-3 overflow-x-auto no-scrollbar pb-2">
              {['Todos', 'Vinhetas', 'Trilhas', 'Efeitos', 'Vozes'].map(
                (cat) => (
                  <button
                    key={cat}
                    className="px-4 py-1.5 rounded-full border border-fd-border text-xs font-medium hover:bg-fd-accent transition-colors whitespace-nowrap"
                  >
                    {cat}
                  </button>
                ),
            ...
            <div className="flex flex-col gap-3 mt-2">
              {loading ? (
                Array.from({ length: 4 }).map((_, i) => (
                  <div
                    key={i}
                    className="h-16 rounded-xl bg-fd-accent animate-pulse"
                  />
                ))
              ) : filtered.length === 0 ? (
                ...
                filtered.map((sound) => (
                  <div
                    key={sound.id}
                    className="group flex items-center justify-between p-4 rounded-xl border border-fd-border bg-white dark:bg-[#121212] hover:border-fd-primary/40 transition-all"
                  >
                    <div className="flex items-center gap-4">
                      <button
                        onClick={() =>
                          setPlayingId(playingId === sound.id ? null : sound.id)
                        }
                      ...
                      <div className="flex flex-col">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-[9px] font-bold uppercase py-0.5 px-1.5 bg-fd-accent rounded text-fd-muted-foreground">
                            {sound.type}
                          </span>
                          <span className="text-[10px] text-fd-muted-foreground flex items-center gap-1">
                            <Clock className="size-3" /> {sound.duration}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="hidden md:flex items-center gap-2">
                      {sound.tags.map((tag) => (
                        <span
                          key={tag}
                          className="text-[10px] text-fd-muted-foreground border border-fd-border px-2 py-0.5 rounded-md italic"
                        >
                          #{tag}
                        </span>
                      ))}
            ...
            {playingId && (
              <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-[calc(100%-2rem)] max-w-2xl bg-white dark:bg-[#1A1A1A] border border-fd-border rounded-2xl shadow-2xl p-4 flex items-center gap-4 z-50 animate-in fade-in slide-in-from-bottom-4 duration-300">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-bold text-fd-foreground truncate">
                    {sounds.find((s) => s.id === playingId)?.title}
                  </p>
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-[10px] text-fd-muted-foreground">
                      {sounds.find((s) => s.id === playingId)?.duration}
                    </span>
                  </div>
                </div>
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  <button
                    ...
                    className="w-8 h-8 rounded-full bg-fd-foreground dark:bg-white flex items-center justify-center text-background dark:text-gray-900"
                  >
