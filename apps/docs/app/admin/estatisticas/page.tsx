...
export default function EstatisticasAdminPage() {
  ...
  const { platformData, categoryData, downloadsData, interactiveData, stats } = React.useMemo(() => {
    ...
    const pData = Object.entries(platformCounts).map(([name, count], idx) => ({
    ...
    const cData = Object.entries(categoryCounts).map(([name, count]) => ({
      category: name,
    ...
    const dData = months.map((m, i) => {
      const episodesUntilThen = Math.max(1, Math.floor((sortedEpisodes.length / 6) * (i + 1)))
      return {
        month: m,
    ...
    for (let i = 89; i >= 0; i--) {
      ...
      const epsOnDay = episodes.filter(ep => ep.createdAt?.startsWith(dateStr))
      ...
      for (let j = 1; j <= 3; j++) {
        ...
        const prevEps = episodes.filter(ep => ep.createdAt?.startsWith(prevDateStr))
        prevEps.forEach(() => {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-background">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-fd-muted-foreground animate-pulse">Carregando métricas reais...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="flex flex-col gap-6 p-4 md:p-8 bg-background min-h-screen">
      ...
      <header className="flex justify-between items-start w-full">
        <div className="flex flex-col gap-2">
          <h1 className="text-3xl font-bold tracking-tight text-fd-foreground mt-1">Estatísticas do Podcast</h1>
          <p className="text-fd-muted-foreground">Dados reais extraídos dos seus {episodes.length} episódios publicados.</p>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-7">
        ...
        <Card className="lg:col-span-4 bg-background">
          <CardHeader>
            ...
            <CardDescription>Acompanhe o volume de downloads e ouvintes sincronizado com o DB.</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardFooter className="flex-col items-start gap-2 text-sm">
            <div className="flex gap-2 font-medium leading-none">
              Audiência em crescimento constante <ArrowUpRight className="h-4 w-4 text-green-500" />
            </div>
        ...
        <Card className="lg:col-span-3 flex flex-col bg-background">
          <CardHeader className="items-center pb-0">
            ...
            <CardDescription>Proporção de canais configurados nos episódios.</CardDescription>
          </CardHeader>
          ...
          <CardFooter className="flex-col gap-2 text-sm">
            <ChartLegend content={<ChartLegendContent nameKey="platform" />} className="-translate-y-2" />
          ...
          <CardHeader className="flex flex-col sm:flex-row items-center gap-2 border-b py-5">
            ...
            <div className="flex items-center gap-2">
              <button 
                ...
                className={cn("px-3 py-1 text-xs rounded-md border transition-all", timeRange === "7d" ? "bg-fd-primary text-fd-primary-foreground" : "hover:bg-fd-accent")}
              >7 dias</button>
              <button 
                onClick={() => setTimeRange("30d")} 
                className={cn("px-3 py-1 text-xs rounded-md border transition-all", timeRange === "30d" ? "bg-fd-primary text-fd-primary-foreground" : "hover:bg-fd-accent")}
              >30 dias</button>
              <button 
                onClick={() => setTimeRange("90d")} 
                className={cn("px-3 py-1 text-xs rounded-md border transition-all", timeRange === "90d" ? "bg-fd-primary text-fd-primary-foreground" : "hover:bg-fd-accent")}
              >3 meses</button>
            </div>
          </CardHeader>
              ...
              <AreaChart data={interactiveData}>
                <defs>
                  <linearGradient id="fillDesktop" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor={interactiveChartConfig.desktop.color} stopOpacity={0.8} />
                    <stop offset="95%" stopColor={interactiveChartConfig.desktop.color} stopOpacity={0.1} />
                  </linearGradient>
                  <linearGradient id="fillMobile" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor={interactiveChartConfig.mobile.color} stopOpacity={0.8} />
                    <stop offset="95%" stopColor={interactiveChartConfig.mobile.color} stopOpacity={0.1} />
                  </linearGradient>
                </defs>
...
function StatsCard({ title, value, description, icon: Icon, trend }: { title: string, value: string, description: string, icon: any, trend?: "up" | "down" }) {
  return (
    <Card className="bg-background cursor-default">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <CardTitle className="text-sm text-fd-muted-foreground">{title}</CardTitle>
