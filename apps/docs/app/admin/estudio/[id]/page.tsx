...
export const aspectRatios: AspectRatioOption[] = [
  {
    label: 'Original',
    subtext: 'First video dimensions',
    ratio: 'auto',
    icon: (
      <svg
        width="20"
  ...
  },
  {
    ...
    icon: (
      <svg
        ...
        strokeWidth="1.5"
      >
        <rect x="2" y="5.5" width="16" height="9" rx="2" />
      </svg>
    ),
  },
  {
    label: '9:16',
    subtext: 'Tiktok, Reels, Shorts',
    ratio: '9 / 16',
    icon: (
      <svg
        width="20"
...
export default function EstudioProjectPage() {
  ...
  const [lastActiveSidebarTab, setLastActiveSidebarTab] =
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setSelectedRatio(
          aspectRatios.find((r) => r.ratio === proj.aspectRatio) ||
            aspectRatios[1],
        );
      } catch {
  ...
  const handleStudioUpload = async (files: FileList | File[]) => {
    ...
    for (const file of Array.from(files)) {
      ...
      const p = async () => {
        ...
        if (!res.ok)
          throw new Error(
            (await res.json().catch(() => ({}))).error || 'Falha no upload',
          );
  ...
  const handleDeleteAsset = async (assetId: string) => {
    const p = async () => {
      const res = await fetch(
        `/api/studio/upload?projectId=${projectId}&assetId=${assetId}`,
        { method: 'DELETE' },
      );
      if (!res.ok)
        throw new Error((await res.json().catch(() => ({}))).error || 'Erro');
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      lastTs = undefined as unknown as number;
    };
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === ' ' || (ctrl && e.key === 'Enter')) {
        ...
        setCurrentTime((p) => Math.min(p + 0.1, 1000));
      else if (e.key === 'ArrowLeft')
        setCurrentTime((p) => Math.max(p - 0.1, 0));
    ...
    (clientX: number) => {
      ...
      const maxTime =
        tracks.length > 0
          ...
          : 30;
      setCurrentTime(
        Math.max(0, Math.min(maxTime, (clientX - rect.left) / 135.6)),
      );
    },
    [tracks],
  );
  ...
  const handleTransformPointerDown = (
    ...
    type: typeof transformRef.current.type,
    track: TimelineTrack,
  ) => {
    e.stopPropagation();
    e.preventDefault();
    ...
    const onMove = (ev: PointerEvent) => {
      ...
      if (s.type === 'move') {
        ...
        const cx =
          rect.left + (rect.width * (s.initialX + s.initialW / 2)) / 100;
        const cy =
          rect.top + (rect.height * (s.initialY + s.initialH / 2)) / 100;
        u.rotation =
          Math.atan2(ev.clientY - cy, ev.clientX - cx) * (180 / Math.PI) + 90;
      } else {
        if (s.type.includes('e')) u.width = Math.max(5, s.initialW + dx);
        if (s.type.includes('w')) {
          const nw = Math.max(5, s.initialW - dx);
          u.width = nw;
          u.x = s.initialX + (s.initialW - nw);
        }
        if (s.type.includes('s')) u.height = Math.max(5, s.initialH + dy);
        if (s.type.includes('n')) {
          const nh = Math.max(5, s.initialH - dy);
          u.height = nh;
          u.y = s.initialY + (s.initialH - nh);
        }
      }
      setTracks((prev) =>
        prev.map((t) => (t.id === s.trackId ? { ...t, ...u } : t)),
      );
    };
    const onUp = () => {
      saveHistory();
  ...
  const handleAddTrack = (
    ...
    if (!isAudio) {
      while (
        tracks.some(
          (t) =>
            (t.layerIndex || 0) === layer &&
            start < t.startTime + t.duration - 0.05 &&
            start + dur > t.startTime + 0.05,
        )
      )
        layer++;
    } else {
      layer =
        (tracks.length > 0
          ? Math.max(...tracks.map((t) => t.layerIndex || 0))
          : 0) + 1;
    }
  ...
  const commonTimelineProps = {
    tracks,
    isTimelineCollapsed,
    toggleTimeline: () => setIsTimelineCollapsed((p) => !p),
    isPlaying,
    setIsPlaying,
    isDragging,
    currentTime,
            ...
            <main className="flex-1 overflow-hidden bg-fd-accent/10 flex flex-col items-center justify-center relative">
              {hasVisualTrack ? (
                              ...
                              {activeVisualTrack && (
                                ...
                                >
                                  <div className="w-full h-full overflow-hidden relative rounded-sm">
                                    {activeVisualTrack.type?.startsWith(
                                      'video/',
                                    ) ? (
                                      <video
                                        src={activeVisualTrack.url}
                                ...
                                selectedTrackIds.includes(
                                  ...
                                  >
                                    ...
                                    {(['nw', 'ne', 'sw', 'se'] as const).map(
                                      (dir) => (
                                        <div
                                          ...
                                          className="absolute bg-white border-2 border-[#3b82f6] pointer-events-auto touch-none rounded-full shadow-md hover:scale-125 transition-transform"
                                          style={{
                                            width: 14,
                                            height: 14,
                                            cursor:
                                    ...
                                    <div
                                      ...
                                      className="absolute bg-white border-2 border-[#3b82f6] pointer-events-auto touch-none rounded shadow-sm"
                                      style={{
                                        left: '50%',
                                        top: -6,
                                        width: 24,
                            ...
                            <div className="flex rounded-[10px] border border-gray-alpha-150 bg-background flex-row divide-x">
                                ...
                                <DropdownMenu.Root>
                                  ...
                                  <DropdownMenu.Portal>
                                    <DropdownMenu.Content
                                      align="start"
                                      sideOffset={8}
                                      className="z-50 min-w-[220px] p-1.5 bg-white dark:bg-black border border-fd-border shadow-xl rounded-2xl animate-in fade-in-0 zoom-in-95"
                                    >
                                      {aspectRatios.map((r) => (
                                        <DropdownMenu.Item
                                          key={r.label}
                                          onSelect={() => setSelectedRatio(r)}
                                          className="flex items-center gap-3 px-2 py-2 rounded-md outline-none hover:bg-fd-accent/50 cursor-pointer"
                                        >
                                          <div className="shrink-0">
                                            {r.icon}
                                          </div>
                              ...
                              <div className="flex items-center p-0.5">
                                <button
                                  ...
                                  className="flex items-center gap-2 px-3 h-9 hover:bg-gray-alpha-100 rounded-lg outline-none"
                                >
                                  <svg
                                    width="20"
                                    height="20"
                                  ...
                                  >
                                    <path
                                      d="M15.0909 2H4.90909C3.30245 2 2 3.30245 2 4.90909V15.0909C2 16.6976 3.30245 18 4.90909 18H15.0909C16.6976 18 18 16.6976 18 15.0909V4.90909C18 3.30245 16.6976 2 15.0909 2Z"
                                      stroke="currentColor"
                                      strokeWidth="1.5"
                ...
                <div className="w-full h-full flex flex-col items-center justify-center text-center p-4">
                  <p className="text-[13px] text-fd-muted-foreground mb-4">
                    Comece a criar: adicione vídeos, fotos e trilhas sonoras
                    para dar vida ao seu podcast.
                  </p>
                  <button
                    onClick={() =>
                      document.getElementById('main-video-upload-proj')?.click()
                    }
                    className="relative inline-flex items-center justify-center whitespace-nowrap font-medium transition-colors bg-background border border-fd-border hover:bg-fd-accent text-foreground h-8 px-4 rounded-lg text-[13px]"
                  >
                    Enviar arquivo
                  </button>
                  <input
                    id="main-video-upload-proj"
                    className="sr-only"
                    accept="video/*,image/*"
                    multiple
                    type="file"
              ...
              {/* Mobile bottom bar */}
              <div
                className="absolute w-full left-0 grow-0 px-2 block md:hidden bottom-[10px]"
                style={{ opacity: 1 }}
              >
                <div className="w-full flex flex-row justify-between">
                  <button
                    onClick={() => setActiveSidebarTab(lastActiveSidebarTab)}
                    className="relative inline-flex items-center justify-center text-sm font-medium border border-gray-alpha-200 text-foreground h-9 px-3 rounded-[10px] bg-background"
                  >
                    Ações
                  </button>
