...
export function StudioHeader({ 
  ...
  return (
    <div className="grid grid-cols-[1fr_auto_1fr] gap-2 items-center w-full overflow-hidden relative px-2 min-w-0 border-b bg-background" style={{ minHeight: '48px' }}>
      {/* Left Section */}
      <div className="flex flex-row items-center gap-2 shrink min-w-0 overflow-hidden">
        <button 
          ...
          className={cn(
            ...
            isProjectPopoverOpen ? "bg-fd-accent text-fd-foreground" : "hover:bg-fd-accent"
          )}
        >
          ...
          <div className="flex relative size-5">
            <div className="absolute [transform:rotate(18deg)] w-full h-full" role="progressbar">
              <svg viewBox="0 0 100 100" className="w-full h-full">
                <circle cx="50" cy="50" r="42" strokeWidth="10" stroke="currentColor" fill="transparent" className="text-fd-accent"></circle>
                <circle cx="50" cy="50" r="42" strokeWidth="10" strokeLinecap="round" strokeDashoffset="66" strokeDasharray="34.32 229.68" stroke="currentColor" fill="transparent" className="text-fd-primary"></circle>
              </svg>
            </div>
            <div className="!absolute left-0 top-0 [transform:rotateY(180deg)] w-full h-full" role="progressbar">
              <svg viewBox="0 0 100 100" className="w-full h-full">
                <circle cx="50" cy="50" r="42" strokeWidth="10" stroke="currentColor" fill="transparent" className="text-fd-accent/30"></circle>
                <circle cx="50" cy="50" r="42" strokeWidth="10" strokeLinecap="round" strokeDashoffset="66" strokeDasharray="203.28 60.72" stroke="currentColor" fill="transparent" className="text-fd-primary/50"></circle>
              </svg>
            </div>
          </div>
          <p className="text-[11px] text-fd-muted-foreground font-medium truncate">8,226 credits remaining</p>
        </div>

        <button aria-label="Open comments" className="relative inline-flex items-center justify-center whitespace-nowrap font-medium transition-colors duration-75 focus-ring bg-transparent text-fd-foreground hover:bg-fd-accent rounded-lg p-0 h-8 w-8">
