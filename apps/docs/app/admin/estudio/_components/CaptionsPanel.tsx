        ...
        <div className="flex items-center gap-1 w-full min-w-0">
          <div className="flex gap-1 items-center">
            <button className="relative items-center justify-center whitespace-nowrap bg-transparent text-foreground shadow-none !p-0 !h-auto rounded-lg text-[13px] font-medium transition-colors flex gap-1 cursor-default">
              Captions
              <span className="text-fd-muted-foreground ml-1 font-normal opacity-60">Classic (legacy)</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col h-full w-full relative overflow-y-auto no-scrollbar">
        <div className="flex flex-col gap-3 py-3.5 pb-10">
          ...
          >
            ...
            {selectedId === 'none' && <Check className="ml-auto w-4 h-4 text-foreground" />}
          </div>
          ...
          {TEMPLATES.map((template) => (
            ...
            >
              ...
              <div className="pt-1 px-1 h-28 w-full overflow-hidden flex">
                <div 
                  className="relative overflow-hidden h-full w-full rounded-lg flex items-center justify-center text-center p-4 bg-black"
                >
                  <p style={template.textStyle}>
                    {template.highlights ? (
                        template.previewText.split(' ').map((word, i) => {
                            const highlight = template.highlights?.find(h => h.text.toLowerCase() === word.toLowerCase());
                            return (
                                <span key={i} style={highlight ? { backgroundColor: highlight.color, color: template.id === 'hype' ? '#fff' : undefined, padding: '0.1em 0.15em', borderRadius: '0.25em' } : { padding: '0.1em 0.15em' }}>
                                    {word}{' '}
                                </span>
                            );
                        })
              ...
              <div className="flex justify-between items-center py-2 px-3">
                <p className="text-foreground font-medium truncate text-[11px]">{template.name}</p>
