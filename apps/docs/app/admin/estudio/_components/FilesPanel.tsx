...
export function FilesPanel({
  ...
  const [recorderAnchor, setRecorderAnchor] = useState<HTMLElement | null>(
    null,
  );
  ...
  return (
    <div className="flex flex-col h-full w-full relative">
      ...
      >
        <span className="text-sm font-medium text-foreground cursor-default select-none">
          Files
        </span>
        ...
        <div className="flex items-center gap-0.5">
          ...
          <button
            ...
            onClick={(e) =>
              setRecorderAnchor(recorderAnchor ? null : e.currentTarget)
            }
            className={cn(
          ...
          >
            ...
            >
              <path
                d="M10 4V12M10 4L13 7M10 4L7 7M17 11.3333V13.4545C17 14.8604 15.8604 16 14.4545 16H5.54545C4.13964 16 3 14.8604 3 13.4545V11.3333"
                stroke="currentColor"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
...
function AssetCard({ file, onAddTrack, onDeleteAsset }: AssetCardProps) {
    ...
    const fetchWaveform = async () => {
      try {
        ...
        for (let i = 0; i < samples; i++) {
          ...
          for (
            let j = 0;
            j < Math.min(blockSize, rawData.length - blockStart);
            j++
          ) {
  ...
  const fakeBars =
    bars.length > 0
      ? bars
      : [
          20, 30, 40, 30, 30, 40, 50, 60, 70, 50, 45, 40, 30, 40, 50, 80, 60,
          70, 80, 90, 85, 90, 70, 60, 50, 70, 80, 90, 85, 75, 60, 50, 40, 30,
          20,
        ];
  ...
  return (
    <div
      className="flex flex-col gap-1 group/asset-item relative"
      ref={cardRef}
    >
      <div
        draggable
      ...
      >
            ...
            file.name.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? (
              <div className="w-full h-full flex items-center justify-center">
                <img
                  src={file.url}
                  alt={file.name}
                  className="max-w-full max-h-full h-auto w-auto rounded-[4px] object-contain pointer-events-none"
                />
              </div>
            ) : (
              <div
                className={cn(
                  'w-full h-full pointer-events-none transition-opacity duration-200 flex items-center justify-center py-4 gap-[1px]',
        ...
        >
          <div
            className={cn(
              'justify-self-end flex items-center transition-all duration-75',
              isMenuOpen
                ? 'opacity-100 translate-x-0'
                : 'md:opacity-0 group-hover/item:opacity-100 md:translate-x-1 group-hover/item:translate-x-0',
            )}
          >
            <div className="flex w-full items-center shadow-sm rounded-md bg-white dark:bg-black h-7 pointer-events-auto border border-fd-border relative">
              ...
              >
                ...
                >
                  <path
                    d="M8.0045 3.5L8.0045 12.5M12.5 8.0045L3.5 8.0045"
                    stroke="currentColor"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                  />
              ...
              >
                <svg
                  ...
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  className="lucide lucide-ellipsis w-3.5 h-3.5"
                >
              ...
              {isMenuOpen && (
                <div
                  role="menu"
                  className="absolute top-full right-0 mt-1 min-w-[7rem] z-50 bg-white dark:bg-black shadow-popover-sm animate-in fade-in-0 zoom-in-95 duration-100 p-0.5 rounded-lg border bg-fd-accent"
                >
                  <div
                    role="menuitem"
                    onClick={() => {
      ...
      <button
        className="flex flex-col items-center justify-center w-full overflow-hidden min-w-0 focus-visible:outline-none rounded-lg mt-0.5"
        onClick={togglePlay}
      >
        <span className="text-foreground font-medium line-clamp-1 w-full inline-flex items-center justify-center text-center cursor-pointer hover:text-fd-muted-foreground transition-colors">
          <div className="relative line-clamp-1 text-center text-[13px] truncate max-w-full">
            {file.name}
          </div>
        </span>
...
function WorkspaceTabContent({
  ...
  return (
    <div
      tabIndex={-1}
      aria-label="Arquivos"
      className="flex flex-col flex-shrink-0 h-full relative focus:outline-none w-full"
    >
      <input
      ...
      <div
        role="presentation"
        className="flex-1 overflow-y-auto min-h-0 relative py-2 pb-20"
      >
              ...
              <div className="flex-shrink-0 flex items-center justify-center w-7 h-7">
                {isFolderOpen ? (
                  <svg
                    ...
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    className="lucide lucide-folder-open text-fd-muted-foreground w-6 h-6"
                  >
                  ...
                  <svg
                    ...
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    className="lucide lucide-folder text-fd-muted-foreground w-6 h-6"
                  >
                  ...
                  assets.map((file) => (
                    <div key={file.id} className="w-full" data-state="open">
                      <div
                        ...
                        data-file-item="true"
                        className="w-full transition-colors duration-150 group/item relative select-none rounded-lg flex items-center gap-2 px-2 py-3 -mt-px first:mt-0 focus:outline-none focus-visible:ring-1 focus-visible:ring-fd-border text-foreground hover:text-foreground hover:bg-fd-accent/50 border-transparent cursor-pointer"
                      >
                        ...
                        <div className="relative line-clamp-1 flex-1 min-w-0">
                          <span className="text-sm font-medium leading-[16.7px] block truncate text-foreground group-hover/item:text-foreground">
                            {file.name}
                          </span>
                        </div>
                        ...
                        <div className="absolute top-1/2 -translate-y-1/2 right-2 z-50">
                          <div
                            data-list-group-actions="true"
                            className="justify-self-end flex items-center md:opacity-0 group-hover/item:opacity-100 transition-all duration-75 md:translate-x-1 group-hover/item:translate-x-0"
                          >
                            <div className="flex w-full items-center shadow-sm rounded-md bg-white dark:bg-black border h-7 pointer-events-auto">
                              <button
                                ...
                                className="relative inline-flex items-center justify-center whitespace-nowrap text-xs px-2 rounded-l-md font-medium h-full border-r border-fd-border hover:bg-fd-accent text-fd-muted-foreground hover:text-foreground transition-colors duration-75 cursor-pointer"
                              ...
                              <button
                                className="relative inline-flex items-center justify-center whitespace-nowrap text-xs px-2 rounded-r-md font-medium h-full hover:bg-fd-accent text-fd-muted-foreground hover:text-foreground transition-colors duration-75 cursor-pointer"
                              ...
                              >
                                <svg
                                  ...
                                  strokeLinecap="round"
                                  strokeLinejoin="round"
                                  className="lucide lucide-eye w-3.5 h-3.5"
                                >
      ...
      {assets.length === 0 ? (
        ...
        <div className="flex flex-col items-center justify-center h-full gap-2.5 py-8 text-center">
          ...
          >
            ...
            >
              <path
                d="M16.8037 9.32258V7.51613C16.8037 6.51845 16.0463 5.70968 15.1119 5.70968H10.7303C10.1646 5.70968 9.63635 5.40782 9.32259 4.90526L8.88635 4.20662C8.41568 3.45279 7.6233 3 6.77479 3H4.53778C3.13621 3 2 4.21317 2 5.70968V15.5792C2 16.3639 2.59575 17 3.33065 17M16.8037 9.32258H7.70703C6.9565 9.32258 6.29569 9.85052 6.08391 10.6193L4.60724 15.9801C4.44068 16.5848 3.92094 17 3.33065 17M16.8037 9.32258H17.1535C17.7182 9.32258 18.1245 9.90218 17.9651 10.4807L16.5265 15.7032C16.3147 16.4721 15.6538 17 14.9033 17H3.33065"
                stroke="currentColor"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
          ...
          <div className="flex flex-col gap-0.5 items-center justify-center select-none">
            ...
            >
              Arraste e solte arquivos de áudio, vídeo ou imagem na janela para
              enviá-los
            </p>
          </div>
        ...
        <div className="flex flex-col grow shrink-0 w-full gap-5 py-3.5 isolate peer empty:hidden h-full">
          <div
            ...
            className="rounded-[10px] border border-gray-alpha-150 bg-background dark:bg-gray-alpha-50 dark:border-gray-alpha-50 flex flex-col !bg-transparent border-none grow"
          >
            <div className="flex flex-col w-full items-center justify-center h-full px-6 py-4 gap-2.5">
              <div
                data-icon="true"
                className="flex shrink-0 w-9 h-9 ring-1 ring-inset ring-gray-alpha-100 rounded-[10px] items-center justify-center text-gray-alpha-500 bg-gray-alpha-100"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
              ...
              <div className="flex flex-col gap-0.5 items-center justify-center select-none">
                ...
                <div
                  data-subtext="true"
                  className="relative line-clamp-4 text-[11px] group-aria-disabled/row:text-gray-400 select-none cursor-default empty:hidden text-gray-400 text-center h-auto"
                >
                  Nenhum arquivo corresponde à sua busca. Tente ajustar os
                  termos de busca.
                </div>
              </div>
        ...
        <div className="flex flex-col grow shrink-0 w-full gap-5 py-3.5 pb-20 isolate peer empty:hidden overflow-y-auto">
          {(() => {
            const audioFiles = filteredFiles.filter(
              (f) =>
