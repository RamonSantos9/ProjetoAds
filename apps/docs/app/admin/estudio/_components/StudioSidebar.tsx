...
const SIDEBAR_TABS: SidebarTab[] = [
  {
    ...
    label: 'Video',
    icon: (
      <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" color="currentColor">
        <path d="M2 6.48276C2 5.11157 3.18197 4 4.64 4H10.36C11.8181 4 13 5.11157 13 6.48276V13.5172C13 14.8885 11.8181 16 10.36 16H4.64C3.18197 16 2 14.8885 2 13.5172V6.48276Z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round"/>
        <path d="M15.7283 6.36305L13 8V12L15.7283 13.637C16.728 14.2368 18 13.5167 18 12.3507V7.64929C18 6.48334 16.728 5.76317 15.7283 6.36305Z" stroke="currentColor" strokeWidth="1.5"/>
      </svg>
    ),
  },
  {
    ...
    icon: (
      <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" color="currentColor">
        <path d="M16.2439 3.125C18.1965 5.07762 18.1965 8.24345 16.2439 10.1961M13.8868 5.48202C14.5377 6.1329 14.5377 7.18817 13.8868 7.83905" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
        <path d="M6.6297 14.3522C7.39353 14.3522 8.15735 13.9694 8.49936 13.3182H8.53357V13.6324C8.56777 14.1237 8.88128 14.375 9.33159 14.375C9.80471 14.375 10.141 14.0894 10.141 13.5353V10.045C10.141 8.81116 9.12069 8 7.55313 8C6.28769 8 5.30725 8.45128 5.02224 9.18246C4.96524 9.30813 4.93674 9.42809 4.93674 9.55948C4.93674 9.93649 5.22745 10.1993 5.63216 10.1993C5.90007 10.1993 6.11098 10.0964 6.29339 9.89079C6.6639 9.41095 6.97171 9.2453 7.47903 9.2453C8.10605 9.2453 8.50507 9.57661 8.50507 10.1479V10.5534L6.96601 10.6448C5.43836 10.7362 4.58333 11.3817 4.58333 12.4956C4.58333 13.5981 5.46686 14.3522 6.6297 14.3522ZM7.17692 13.1468C6.6183 13.1468 6.24209 12.8612 6.24209 12.4042C6.24209 11.9701 6.5955 11.6902 7.21682 11.6445L8.50507 11.5645V12.0101C8.50507 12.6727 7.90654 13.1468 7.17692 13.1468Z" fill="currentColor"/>
        <path d="M10 3.33333H5C3.61929 3.33333 2.5 4.45262 2.5 5.83333V14.1667C2.5 15.5474 3.61929 16.6667 5 16.6667H15C16.3807 16.6667 17.5 15.5474 17.5 14.1667V13.3333" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
      </svg>
    ),
  },
  {
  ...
  },
  {
    ...
    label: 'Text',
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-type w-4 h-4">
...
interface StudioSidebarProps {
  ...
  onAddTrack?: (file: { id: string; name: string; url?: string; type: string; size: number }, duration: number) => void;
  activeTab: string | null;
  setActiveTab: React.Dispatch<React.SetStateAction<string | null>>;
  assets?: UploadedFile[];
}

export function StudioSidebar({ onUpload, onDeleteAsset, onAddTrack, activeTab, setActiveTab, assets = [] }: StudioSidebarProps) {
    ...
    <>
      ...
      <div className="z-20 sticky top-0 left-0 h-full hidden md:flex shrink-0">
        ...
        >
          ...
          <div className="w-full h-full overflow-hidden border-r" style={{ width: activeTab ? panelWidth : 0 }}>
            <div className="h-full flex flex-col shrink-0" style={{ minWidth: PANEL_MIN }}>
              <div className="flex flex-col w-full h-full overflow-y-auto group/sidebar-content bg-background relative">
                {activeTab === 'files' && <FilesPanel onUpload={onUpload} onDeleteAsset={onDeleteAsset} onAddTrack={onAddTrack} assets={assets} />}
                {activeTab === 'captions' && <CaptionsPanel />}
                {activeTab && activeTab !== 'files' && activeTab !== 'captions' && (
      ...
      <div className="md:hidden border-t bg-background w-full z-40 flex flex-col shrink-0">
        <div className="flex h-13 w-full items-center relative overflow-x-auto no-scrollbar border-b">
           <div role="tablist" aria-orientation="horizontal" className="flex gap-1 px-3.5 py-1.5 w-full">
            {SIDEBAR_TABS.map((tab) => {
              const isActive = activeTab === tab.id;
              return (
                <button 
                  ...
                  className={cn(
                    ...
                    isActive ? "bg-fd-accent text-fd-foreground" : "text-gray-500 hover:bg-fd-accent/50"
                  )}
                >
      ...
      {activeTab && (
        <>
          ...
          >
            {/* Handle bar */}
            <div className="py-2 shrink-0 cursor-grab active:cursor-grabbing" onClick={() => setActiveTab(null)}>
              ...
              <div className="flex flex-col w-full flex-1 overflow-y-auto no-scrollbar bg-background">
                ...
                {activeTab && activeTab !== 'files' && activeTab !== 'captions' && (
                  <div className="flex flex-col items-center justify-center h-full gap-4 text-fd-muted-foreground p-10 text-center">
                    <p className="text-sm font-medium">{SIDEBAR_TABS.find(t => t.id === activeTab)?.label}</p>
                    <p className="text-xs">Recurso em desenvolvimento para mobile</p>
                  </div>
                )}
              </div>
