  ...
  try {
    ...
    if (!apiKey) {
      return NextResponse.json(
        { error: 'Chave de API do PodcastAds não configurada' },
        { status: 500 },
      );
    }
    ...
    const finalVoices = targetNames.map((name, index) => {
      ...
      return {
        ...
        name: name,
        description: realVoice
          ? realVoice.description ||
            realVoice.labels?.accent ||
            descriptions[index]
          : descriptions[index],
        image: images[index],
        previewUrl: realVoice
          ? realVoice.preview_url || defaultPreviewUrls[index]
          : defaultPreviewUrls[index],
