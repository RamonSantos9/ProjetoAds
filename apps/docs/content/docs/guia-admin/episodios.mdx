...
<Callout type="warn">
  - **Integridade de Arquivos**: O sistema valida a tipagem MIME para garantir
  que apenas áudios compatíveis sejam publicados. - **Slugs Amigáveis**: URLs
  são normalizadas para SEO (Search Engine Optimization) no momento da criação.
  - **Persistência Federada**: Alterações no `/admin` são propagadas
  instantaneamente para o banco de dados (PostgreSQL), refletindo em tempo real no
  Dashboard do Ouvinte.
</Callout>
...
3. Limpe o cache do Next.js se a alteração for estrutural no banco de dados.
