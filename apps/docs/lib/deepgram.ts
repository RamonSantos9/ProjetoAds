  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401 || response.status === 402 || response.status === 429) {
        return generateMockTranscription(title, summary, `Deepgram ${response.status}`);
      }
    ...
    if (utterances.length > 0) {
      formattedText = utterances.map((u: any) => {
        const minutes = Math.floor(u.start / 60).toString().padStart(2, '0');
