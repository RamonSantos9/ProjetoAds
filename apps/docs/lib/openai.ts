...
export async function transcribeAudio(
  ...
  if (!OPENAI_API_KEY) {
    console.warn(
      '[OpenAI] Chave de API não encontrada. Usando modo de simulação.',
    );
  ...
  try {
    ...
    const blob = new (globalThis as any).Blob([audioFile], {
      type: 'audio/mpeg',
    });
    ...
    if (!response.ok) {
      ...
      if (response.status === 402 || response.status === 429) {
        console.warn(
          `[OpenAI] Erro de saldo (${response.status}). Ativando fallback para teste local.`,
        );
        return generateMockTranscription(
          title,
          summary,
          `API Sem Saldo (${response.status})`,
        );
      }
