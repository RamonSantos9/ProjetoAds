...
export function EditEpisodeModal({ isOpen, onClose, episode, onSave }: EditEpisodeModalProps) {
  ...
  useEffect(() => {
    if (episode && isOpen) {
      ...
      const allGuestNames = episode.guests?.map(g => typeof g === 'object' && g !== null ? g.name : g) || [];
    ...
    >
      <div className="flex flex-col max-h-[90vh] bg-fd-background border border-border rounded-2xl w-full max-w-[550px] animate-in zoom-in-95 duration-200 overflow-hidden">
        ...
        <header className="flex items-center justify-between px-6 py-5 border-b border-border bg-fd-muted/30">
          <div>
            <h2 className="font-bold text-xl text-fd-foreground">Editar Episódio</h2>
            <p className="text-xs text-fd-muted-foreground mt-0.5">Atualize as informações do seu conteúdo profissional.</p>
          </div>
          <button onClick={onClose} className="p-2 rounded-full hover:bg-fd-muted transition-colors text-fd-muted-foreground hover:text-fd-foreground">
            <X className="size-5" />
          </button>
        </header>
        ...
        <main className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          <form id="edit-episode-form" className="space-y-6" onSubmit={handleSubmit}>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        <p className="text-sm font-medium text-foreground">
                          {audioFile ? audioFile.name : (episode?.audioUrl ? 'Manter Áudio Atual' : 'Selecionar Áudio')}
                        </p>
                      </div>
                    </div>
