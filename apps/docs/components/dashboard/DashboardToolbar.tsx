  ...
  return (
    <div className="relative w-full sm:w-auto" ref={containerRef}>
      ...
      >
        ...
        <ChevronRight className={cn("size-4 text-fd-muted-foreground transition-transform", isOpen ? "rotate-270" : "rotate-90")} />
      </button>
      {isOpen && (
...
export function ToolbarExport({ onExport }: { onExport?: (f: 'JSON' | 'CSV') => void }) {
  const [isOpen, setIsOpen] = useState(false);
  const containerRef = useRef<HTMLDivElement>(null);
    ...
    <div className="relative w-full sm:w-auto" ref={containerRef}>
      ...
      {isOpen && (
         <div className="absolute top-[calc(100%+4px)] right-0 w-full sm:w-40 bg-[#FFFFFF] dark:bg-[#121212] border rounded-lg z-50 overflow-hidden shadow-xl animate-in fade-in zoom-in-95">
           <button onClick={() => { onExport?.('JSON'); setIsOpen(false); }} className="w-full text-left px-4 py-2.5 text-xs font-semibold hover:bg-fd-accent border-b border-fd-border text-fd-foreground">JSON</button>
           <button onClick={() => { onExport?.('CSV'); setIsOpen(false); }} className="w-full text-left px-4 py-2.5 text-xs font-semibold hover:bg-fd-accent text-fd-foreground">CSV</button>
         </div>
      )}
    </div>
  );
