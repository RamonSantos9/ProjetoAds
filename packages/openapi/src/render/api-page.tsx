  ...
  return (
    <ctx.renderer.Root ctx={ctx}>
      {operations?.map((item) => {
        ...
        if (!pathItem)
          throw new Error(
            `[XispeDocs OpenAPI] Path not found in OpenAPI schema: ${item.path}`,
          );

        const operation = pathItem[item.method];
        if (!operation)
          throw new Error(
            `[XispeDocs OpenAPI] Method ${item.method} not found in operation: ${item.path}`,
          );
      ...
      {webhooks?.map((item) => {
        ...
        if (!webhook)
          throw new Error(
            `[XispeDocs OpenAPI] Webhook not found in OpenAPI schema: ${item.name}`,
          );

        const hook = webhook[item.method];
        if (!hook)
          throw new Error(
            `[XispeDocs OpenAPI] Method ${item.method} not found in webhook: ${item.name}`,
          );
