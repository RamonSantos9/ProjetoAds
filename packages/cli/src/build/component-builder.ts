...
export function createComponentBuilder(
  ...
  for (const comp of registry.components) {
    for (const file of comp.files) {
      if (fileToComponent.has(file.path))
        console.warn(
          `O mesmo arquivo ${file.path} existe em vários componentes, você deve fazer o arquivo compartilhado um componente separado.`,
        );
  ...
  return {
    ...
    getDepInfo(name: string):
      ...
      console.warn(`a informação de dependência para ${name} não pode ser encontrada`);
    },
