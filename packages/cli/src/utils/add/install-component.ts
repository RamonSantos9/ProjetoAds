  ...
  function buildFileList(downloaded: DownloadedComponents): OutputFile[] {
    ...
    for (const item of downloaded) {
      for (const file of item.files) {
        ...
        if (map.has(filePath)) {
          console.warn(
            `notado arquivo de saída duplicado para ${filePath}, ignorando por enquanto.`,
          );
          continue;
        }
      ...
      for (const file of fileList) {
        ...
        if (status === 'need-update') {
          const override = await confirm({
            message: `Você deseja substituir ${outPath}?`,
            initialValue: false,
          });
    ...
    async download(name: string): Promise<DownloadedComponents> {
      ...
      const comp = validateRegistryComponent(
        await resolver(`${name}.json`).catch((e) => {
          log.error(`componente ${name} não encontrado:`);
          log.error(String(e));
          process.exit(1);
        }),
      );
    ...
    transform(filePath: string, file: OutputFile, fileList: OutputFile[]) {
      ...
      transformReferences(sourceFile, (specifier) => {
        ...
        if (specifier.startsWith(prefix)) {
          ...
          if (!target) {
            console.warn(
              `não foi possível encontrar o arquivo referenciado de ${specifier}`,
            );
            return specifier;
          }
...
export function remoteResolver(url: string): Resolver {
  return async (file) => {
    ...
    if (!res.ok) {
      throw new Error(`falha ao buscar ${url}/${file}: ${res.statusText}`);
    }
...
export function localResolver(dir: string): Resolver {
  return async (file) => {
    ...
    return await fs
      ...
      .catch((e) => {
        throw new Error(`falha ao resolver o arquivo local "${filePath}"`, {
          cause: e,
        });
      });
  };
