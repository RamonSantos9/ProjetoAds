...
// Mock fetch for remote image tests
vi.stubGlobal(
  'fetch',
  vi.fn(async () => {
    return {
      ok: true,
      arrayBuffer: async () =>
        readFileSync(path.resolve(cwd, './fixtures/test.png')),
    };
  }),
);
...
test('Remark Image: Without Import', async () => {
  ...
  const result = await remark()
    ...
    .use(remarkMdx)
    .process({ value: content, path: file });
...
test('Remark Image: `publicDir` with URL', async () => {
  const file = path.resolve(cwd, './fixtures/remark-image-public-dir.md');
  const content = readFileSync(file);
  const result = await remark()
    .use(remarkImage, {
      publicDir: 'https://xispedocs.com.br',
      useImport: false,
    })
    .use(remarkMdx)
    .process({ value: content, path: file });
