...
import { buildPlugins, type LoaderPlugin } from './plugins';
...
export interface LoaderOutput<Config extends LoaderConfig> {
  ...
  /**
   * Serialize page tree for server components
   */
  serializePageTree: (tree: PageTree.Root) => PageTree.Root;
}
...
function createOutput({
  ...
  return {
    ...
    serializePageTree(tree) {
      return JSON.parse(JSON.stringify(tree)) as PageTree.Root;
    },
  };
}
