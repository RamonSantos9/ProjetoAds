...
export async function highlightHast(
  ...
  if (typeof engine === 'string') {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.warn(
        '[XispeDocs `highlight()`] Avoid passing `engine` directly. For custom engines, use `shiki` directly instead.',
      );
    }
  }
...
/**
 * Get Shiki highlighter instance of XispeDocs (mostly for internal use, don't recommend you to use it).
 *
