...
import { type BaseCreate, fromConfigBase } from './base';

export function fromConfig<Config>(): BaseCreate<Config> {
  ...
  return {
    ...
    docs(name, docs, meta) {
      return {
        ...base.docs(name, docs, meta),
        createClientLoader(options) {
          return createClientLoader(docs, {
            id: name as string,
            ...options,
          });
        },
...
export interface ClientLoader<Frontmatter, Props> {
  ...
  useContent: (path: string) => ReactNode;
}
...
export function createClientLoader<Frontmatter, Props = object>(
  ...
  return {
    ...
    useContent(path) {
      const Component = this.getComponent(path) as any;
