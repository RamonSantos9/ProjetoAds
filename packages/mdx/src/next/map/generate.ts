...
import { xispedocsMatter } from '@/utils/xispedocs-matter';
import {
  getImportCode,
...
export async function generateJS(
  ...
  const lines: string[] = [
    getImportCode({
      type: 'named',
      names: ['_runtime'],
      specifier: '@xispedocs/mdx/runtime/next',
    }),
    getImportCode({
      type: 'namespace',
  ...
  async function getAsyncEntries(collection: DocCollection, files: FileInfo[]) {
    if (!asyncInit) {
      lines.unshift(
        getImportCode({
          type: 'named',
          specifier: '@xispedocs/mdx/runtime/async',
    ...
    const entries = files.map(async (file) => {
      ...
      const parsed = xispedocsMatter(content);
