...
import { HideIfEmpty } from '@xispedocs/core/hide-if-empty';
import {
  getSidebarTabs,
...
export function DocsLayout(props: DocsLayoutProps) {
  ...
  function sidebar() {
    ...
    const iconLinks = links.filter((item: any) => item.type === 'icon');
    ...
    const viewport = (
      <SidebarViewport>
        {links
          .filter((item: any) => item.type !== 'icon')
          .map((item: any, i, arr) => (
            <SidebarLinkItem
              key={i}
              item={item}
...
function NavbarLinkItem({
  ...
  return (
    <LinkItem item={item as any} {...props}>
      {item.text}
    </LinkItem>
  );
}
