...
export { type BreadcrumbProps, type FooterProps, PageLastUpdate, PageBreadcrumb, PageFooter, PageTOC, PageTOCPopover, PageTOCPopoverContent, PageTOCPopoverTrigger };
...
export function PageRoot({ toc = false, children, ...props }: RootProps) {
  const content = (
    <div
      ...
      {...props}
      className={cn(
        'flex flex-1 w-full mx-auto max-w-(--fd-page-width) pt-(--fd-tocnav-height) pe-(--fd-toc-width) [grid-area:main]',
        props.className,
      )}
    >
      {children}
